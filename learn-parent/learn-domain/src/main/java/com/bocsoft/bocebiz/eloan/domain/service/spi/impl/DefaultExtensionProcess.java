...
public class DefaultExtensionProcess implements ExtensionProcess {
	...
	public DefaultExtensionProcess(Object bean, Executor executor) {
		...
		if (config == null) {
			throw new ExtensionProcessConfigException("The extension process must be configed with annotation : " + ExtensionProcessConfiguration.class.getName());
		}
		this.target = bean;
	...
	private Throwable syncfireProcessExtension(Method method, Object... args) {
		...
		try {
			...
			log.error("error occured while dispatch process extension named {}, the exception message is {}", target.getClass().getName(), e.getMessage());
